...
from emma_datasets.datamodels.datasets import SimBotMissionInstance
from emma_datasets.datamodels.datasets.simbot import load_simbot_mission_data
...
def test_can_load_simbot_mission_data(simbot_instances_path: Path) -> None:
    ...
    instances = parse_obj_as(
        list[SimBotMissionInstance], load_simbot_mission_data(simbot_instances_path)
    )
